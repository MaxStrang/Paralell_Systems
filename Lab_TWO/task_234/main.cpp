...
int main(int argc, char** argv) {
	...
	if(rank==0) {
		...
		printf("number of processes = %d of which 2 run the functions in() and out()\n"
		       "completion time = %.2f sec\n"
			    "avg service time = %.2f sec\n"
			    "avg latency = %.2f sec\n", 
			    required_comm_size, completion_time, avg_service_time, avg_latency);
	}
	//quit MPI env
	MPI_Finalize();
	//exit
